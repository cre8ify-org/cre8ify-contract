...
contract CCP {
    ...
    struct vars {
        ...
        mapping(address => uint256) fetchContentTimestamp;
    ...
    event ContentCreated(uint256 indexed id, address indexed creator, string creatorUsername, uint256  indexed timestamp);
    event ContentMonetized(uint256 id, address creator);
    event ContentDemonetized(uint256 id, address creator);
    ...
    function createContent(
        ...
        emit ContentCreated(appVars.contentCount, msg.sender, username, block.timestamp);
    }
    ...
    function fetchAllContent() public view returns (ContentItem[] memory) {
        return appVars.contentsArray;
    }

    function fetch20Content(uint256 lastId) public returns (ContentItem[] memory, uint256) {
        ...
        appVars.fetchContentTimestamp[msg.sender] = block.timestamp;
        ...
        bool isUserSubscribed= subscriptionContract.isUserSubscribed(msg.sender);

        while (fetchedCount < 20 && endIndex > 0) {
            ContentItem memory content = appVars.contentsArray[startIndex];

            if (!content.isDeleted) {
                if (content.isMonetized && isUserSubscribed){
