...
contract Vault {
   ...
   function tipCreator(uint256 amount, address _Tipper, address _creator) public {
   ...
   function withdrawStake(uint256 amount, address _staker) external {
       require(stakedBalances[_staker] >= amount, "Insufficient staked balance");
   ...
   function CreatorPayout(uint256 amount, address _creator) external {
       require(creatorAccured[_creator] >= amount, "Insufficient staked balance");
       creatorAccured[_creator] -= amount;
